...
public class ActionBarContextView extends AbsActionBarView {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (MeasureSpec.getMode(widthMeasureSpec) != 1073741824) {
            ...
            int childSpecHeight = MeasureSpec.makeMeasureSpec(height, Action.UNDEFINED_DURATION);
            if (this.mClose != null) {
            ...
            if (this.mCustomView != null) {
                ...
                int customWidthMode = lp2.width != -2 ? 1073741824 : Action.UNDEFINED_DURATION;
                if (lp2.width >= 0) {
                    customWidth = Math.min(lp2.width, availableWidth);
                } else {
                    customWidth = availableWidth;
                }
                int customHeightMode = lp2.height != -2 ? 1073741824 : Action.UNDEFINED_DURATION;
                if (lp2.height >= 0) {
