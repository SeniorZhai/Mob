...
class AppCompatDelegateImplV7 extends AppCompatDelegateImplBase implements Callback, LayoutInflaterFactory {
    ...
    private final Runnable mInvalidatePanelMenuRunnable = new Runnable() {
        public void run() {
            ...
            if ((AppCompatDelegateImplV7.this.mInvalidatePanelMenuFeatures & MpegAudioHeader.MAX_FRAME_SIZE_BYTES) != 0) {
    ...
    boolean onKeyUp(int keyCode, KeyEvent event) {
        switch (keyCode) {
            case CommonUtils.DEVICE_STATE_DEBUGGERATTACHED /*4*/:
                ...
                break;
            case com.mobcrush.mobcrush.R.styleable.Theme_colorPrimaryDark /*82*/:
        ...
        if (!st.isOpen && !isDestroyed()) {
            ...
            if (cb == null || cb.onMenuOpened(st.featureId, st.menu)) {
                ...
                if (wm != null && preparePanel(st, event)) {
                    ...
                    st.isHandled = false;
                    WindowManager.LayoutParams lp2 = new WindowManager.LayoutParams(width, -2, st.x, st.y, CloseFrame.PROTOCOL_ERROR, 8519680, -3);
