...
public class ItemTouchHelper extends ItemDecoration implements OnChildAttachStateChangeListener {
                ...
                if (viewHolder != null) {
                    switch (action) {
                        ...
                        case TimePickerDialog.ENABLE_PICKER_INDEX /*3*/:
                            if (ItemTouchHelper.this.mVelocityTracker != null) {
                                ItemTouchHelper.this.mVelocityTracker.computeCurrentVelocity(Constants.UPDATE_COFIG_INTERVAL, (float) ItemTouchHelper.this.mRecyclerView.getMaxFlingVelocity());
                            }
                            ItemTouchHelper.this.select(null, ItemTouchHelper.ACTION_STATE_IDLE);
                            ItemTouchHelper.this.mActivePointerId = ItemTouchHelper.ACTIVE_POINTER_ID_NONE;
                            return;
    ...
    private int mOverdrawChildPosition = ACTIVE_POINTER_ID_NONE;
    ...
    private final float[] mTmpPosition = new float[DOWN];
    ...
    private void postDispatchSwipe(final RecoverAnimation anim, final int swipeDir) {
        this.mRecyclerView.post(new Runnable() {
            public void run() {
                if (ItemTouchHelper.this.mRecyclerView != null && ItemTouchHelper.this.mRecyclerView.isAttachedToWindow() && !anim.mOverridden && anim.mViewHolder.getAdapterPosition() != ItemTouchHelper.ACTIVE_POINTER_ID_NONE) {
    ...
    private int checkHorizontalSwipe(ViewHolder viewHolder, int flags) {
        if ((flags & 12) != 0) {
            ...
            if (this.mVelocityTracker != null && this.mActivePointerId > ACTIVE_POINTER_ID_NONE) {
                ...
                int velDirFlag = xVelocity > 0.0f ? RIGHT : LEFT;
    ...
    private int checkVerticalSwipe(ViewHolder viewHolder, int flags) {
        if ((flags & 3) != 0) {
            ...
            if (this.mVelocityTracker != null && this.mActivePointerId > ACTIVE_POINTER_ID_NONE) {
                ...
                int velDirFlag = yVelocity > 0.0f ? DOWN : UP;
