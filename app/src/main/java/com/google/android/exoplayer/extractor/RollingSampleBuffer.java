...
final class RollingSampleBuffer {
    ...
    private final ParsableByteArray scratch = new ParsableByteArray((int) INITIAL_SCRATCH_SIZE);
    ...
    public int appendData(DataSource dataSource, int length) throws IOException {
        ensureSpaceForWrite();
        int remainingAllocationCapacity = this.allocationLength - this.lastAllocationOffset;
        int bytesRead = dataSource.read(this.lastAllocation.data, this.lastAllocation.translateOffset(this.lastAllocationOffset), length != -1 ? Math.min(length, remainingAllocationCapacity) : remainingAllocationCapacity);
        if (bytesRead == -1) {
            return -1;
        }
        this.lastAllocationOffset += bytesRead;
        this.totalBytesWritten += (long) bytesRead;
        return bytesRead;
    }

    public int appendData(ExtractorInput input, int length) throws IOException, InterruptedException {
        ensureSpaceForWrite();
        int thisWriteLength = Math.min(length, this.allocationLength - this.lastAllocationOffset);
        input.readFully(this.lastAllocation.data, this.lastAllocation.translateOffset(this.lastAllocationOffset), thisWriteLength);
        this.lastAllocationOffset += thisWriteLength;
        this.totalBytesWritten += (long) thisWriteLength;
    ...
    public void appendData(ParsableByteArray buffer, int length) {
        ...
        while (remainingWriteLength > 0) {
            ensureSpaceForWrite();
            int thisWriteLength = Math.min(remainingWriteLength, this.allocationLength - this.lastAllocationOffset);
            buffer.readBytes(this.lastAllocation.data, this.lastAllocation.translateOffset(this.lastAllocationOffset), thisWriteLength);
            this.lastAllocationOffset += thisWriteLength;
