...
public class Draft_76 extends Draft_75 {
    ...
    private static byte[] getPart(String key) throws InvalidHandshakeException {
        try {
            long keyNumber = Long.parseLong(key.replaceAll("[^0-9]", BuildConfig.FLAVOR));
    ...
    public HandshakeState acceptHandshakeAsServer(ClientHandshake handshakedata) {
        if (handshakedata.getFieldValue(HttpHeaders.UPGRADE).equals("WebSocket") && handshakedata.getFieldValue(HTTP.CONN_DIRECTIVE).contains(HttpHeaders.UPGRADE) && handshakedata.getFieldValue("Sec-WebSocket-Key1").length() > 0 && !handshakedata.getFieldValue("Sec-WebSocket-Key2").isEmpty() && handshakedata.hasFieldValue("Origin")) {
            return HandshakeState.MATCHED;
        }
        return HandshakeState.NOT_MATCHED;
    }
    ...
    public ClientHandshakeBuilder postProcessHandshakeRequestAsClient(ClientHandshakeBuilder request) {
        ...
        request.put(HTTP.CONN_DIRECTIVE, HttpHeaders.UPGRADE);
    ...
    public HandshakeBuilder postProcessHandshakeResponseAsServer(ClientHandshake request, ServerHandshakeBuilder response) throws InvalidHandshakeException {
        ...
        response.put(HTTP.CONN_DIRECTIVE, request.getFieldValue(HTTP.CONN_DIRECTIVE));
        response.put("Sec-WebSocket-Origin", request.getFieldValue("Origin"));
        response.put("Sec-WebSocket-Location", "ws://" + request.getFieldValue(HTTP.TARGET_HOST) + request.getResourceDescriptor());
    ...
    public List<Framedata> translateFrame(ByteBuffer buffer) throws InvalidDataException {
        ...
        if (this.currentFrame == null) {
            ...
            if (this.currentFrame.hasRemaining()) {
                ...
                frames.add(new CloseFrameBuilder(Constants.UPDATE_COFIG_INTERVAL));
                return frames;
            } else {
                throw new InvalidFrameException();
            }
