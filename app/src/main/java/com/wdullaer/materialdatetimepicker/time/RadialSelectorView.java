...
public class RadialSelectorView extends View {
    ...
    public void initialize(Context context, boolean is24HourMode, boolean hasInnerCircle, boolean disappearsOut, int selectionDegrees, boolean isInnerCircle) {
        ...
        this.mPaint.setColor(res.getColor(R.color.mdtp_accent_color));
        ...
        this.mAnimationRadiusMultiplier = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
        if (disappearsOut) {
            i2 = -1;
        } else {
            i2 = 1;
        }
        this.mTransitionMidRadiusMultiplier = (((float) i2) * 0.05f) + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
        if (disappearsOut) {
            i = 1;
        }
        this.mTransitionEndRadiusMultiplier = (0.3f * ((float) i)) + DefaultRetryPolicy.DEFAULT_BACKOFF_MULT;
    ...
    public int getDegreesFromCoords(float pointX, float pointY, boolean forceLegal, Boolean[] isInnerCircle) {
        ...
        } else if (!forceLegal && ((int) Math.abs(hypotenuse - ((double) this.mLineLength))) > ((int) (((float) this.mCircleRadius) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - this.mNumbersRadiusMultiplier)))) {
            return -1;
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (getWidth() != 0 && this.mIsInitialized) {
            ...
            if ((i | z) != 0) {
                this.mPaint.setAlpha(SELECTED_ALPHA_THEME_DARK);
            ...
            this.mPaint.setAlpha(SELECTED_ALPHA_THEME_DARK);
            this.mPaint.setStrokeWidth(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    ...
    public ObjectAnimator getDisappearAnimator() {
        if (this.mIsInitialized && this.mDrawValuesReady) {
            Keyframe kf0 = Keyframe.ofFloat(0.0f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            Keyframe kf1 = Keyframe.ofFloat(DefaultLoadControl.DEFAULT_LOW_BUFFER_LOAD, this.mTransitionMidRadiusMultiplier);
            Keyframe kf2 = Keyframe.ofFloat(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, this.mTransitionEndRadiusMultiplier);
            PropertyValuesHolder radiusDisappear = PropertyValuesHolder.ofKeyframe("animationRadiusMultiplier", new Keyframe[]{kf0, kf1, kf2});
            kf0 = Keyframe.ofFloat(0.0f, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            kf1 = Keyframe.ofFloat(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, 0.0f);
            PropertyValuesHolder fadeOut = PropertyValuesHolder.ofKeyframe("alpha", new Keyframe[]{kf0, kf1});
            ObjectAnimator disappearAnimator = ObjectAnimator.ofPropertyValuesHolder(this, new PropertyValuesHolder[]{radiusDisappear, fadeOut}).setDuration((long) HttpStatus.SC_INTERNAL_SERVER_ERROR);
    ...
    public ObjectAnimator getReappearAnimator() {
        if (this.mIsInitialized && this.mDrawValuesReady) {
            int totalDuration = (int) (((float) 500) * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT + 0.25f));
            float delayPoint = (((float) 500) * 0.25f) / ((float) totalDuration);
            float midwayPoint = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - ((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - delayPoint) * DefaultLoadControl.DEFAULT_LOW_BUFFER_LOAD);
            ...
            Keyframe kf3 = Keyframe.ofFloat(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
            ...
            kf2 = Keyframe.ofFloat(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
